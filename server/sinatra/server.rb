...
class Controller < Sinatra::Base
  ...
  def identity
    ...
    Store.get_hash(real_path) || Store.put_hash(real_path, FileStore.get_hash(default_path))
  end
    ...
    Store.page_metadata(farm_dir, max_pages).each do |page|
      #freshness_score = freshness page['updated_at']
        ...
        unless collection_hash
          url = "http://#{page['site']}#{request.port == 80 ? '' : ":#{request.port}"}/view/home"
        ...
        page_hash = pages_hashes.find{ |page_hash| page_hash['name'] == page['name'] }
        unless page_hash
          page_hash = {"name" => page['name'], "size" => 1}
